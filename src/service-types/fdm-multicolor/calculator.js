...
export function calculateFdmMultiColorQuote(inputs = {}, catalogs = {}) {
  ...
  // Material (8 slots) – CSV only, no custom materials
let totalMaterialCost = 0;

for (let i = 1; i <= 8; i++) {
  const materialIdKey = `materialId${i}`;
  const weightKey = `materialWeightGrams${i}`;

  const matId = inputs[materialIdKey];
  const weightGrams = toNumber(inputs[weightKey], 0);

  if (!matId || weightGrams <= 0) continue;

  const selectedMaterial =
    materials.find((m) => String(m.material_id) === String(matId)) || null;

  if (!selectedMaterial) continue;

  const adjustedPricePerKg = toNumber(
    selectedMaterial.adjusted_price_per_kg,
    0
  );
  const kg = weightGrams / 1000;

  totalMaterialCost += kg * adjustedPricePerKg;
}

  var materialCost = totalMaterialCost
